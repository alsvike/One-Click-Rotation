...
class KeyListener(QThread):
        ...
        try:
            while self.running:
                if not self._is_stopping:
                    ...
                    if self.running and not self._is_stopping:
    ...
    def stop(self):
        ...
        self.running = False
        # Use a different key to break the wait loop
...
class ConfigDialog(QDialog):
    def __init__(self, parent=None, existing_config=None):
        ...
        self.setWindowTitle("Create New Configuration" if not existing_config else "Edit Configuration")
    ...
    def load_existing_config(self):
        ...
        # Set trigger key
        trigger_index = self.trigger_combo.findText(self.existing_config['trigger'])
        if trigger_index >= 0:
            self.trigger_combo.setCurrentIndex(trigger_index)
        
        # Remove default first key widget
        if self.sequence_widgets:
            self.remove_sequence_key()
        
        # Add existing sequence keys
        for key in self.existing_config['sequence']:
            key_combo = QComboBox()
            self.populate_key_dropdown(key_combo)
            key_index = key_combo.findText(key)
            if key_index >= 0:
                key_combo.setCurrentIndex(key_index)
            self.sequence_widgets.append(key_combo)
            self.sequence_layout.addWidget(key_combo)
...
class MainWindow(QMainWindow):
    ...
    def edit_config(self):
        ...
        if current_row >= 0:
            ...
            current_config = self.configs[current_row]
            dialog = ConfigDialog(self, existing_config=current_config)
            
            if dialog.exec_():
                updated_config = dialog.get_config()
                if not updated_config['name'] or not updated_config['sequence'] or not updated_config['trigger']:
                    QMessageBox.warning(self, "Error", "All fields are required!")
                    return
                
                # Validate all keys
                invalid_keys = []
                for key in updated_config['sequence']:
                    if not is_valid_key(key):
                        invalid_keys.append(key)
                ...
                # Update config
                self.configs[current_row] = updated_config
                self.config_list.item(current_row).setText(updated_config['name'])
                self.save_configs()
                self.log_message(f"Updated configuration: {updated_config['name']}")
...
def closeEvent(self, event):
        # Ensure clean shutdown of key listener if running
        if self.key_listener and self.key_listener.isRunning():
            self.key_listener.stop()
            self.key_listener.wait()  # Wait for thread to finish
        event.accept()
